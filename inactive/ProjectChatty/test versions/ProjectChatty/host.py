...
while c != -1:
    while c == 0:
        time.sleep(1);
        setupH();
        tSend = threading.Thread(target=send_recv.sendMsg,
                                 args=(conn, namething, name, name1, sendReadAlerts, theirEIP, "h",));
        tRecv = threading.Thread(target=send_recv.recvMsg,
                                 args=(conn, namething, name, name1, sendReadAlerts, "h",));
        tRecv.start();
        tSend.start();
            # need to get rid
