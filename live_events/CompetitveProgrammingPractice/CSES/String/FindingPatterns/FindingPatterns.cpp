...
int k, seen[K], sz = 1, wordidx[K], adj[K][26], fail[K], dict[K], par[K], topar[K];
bool ans[K];
string tar;
...
int gen(int node) {
    ...
    int cur = par[node];
    if (fail[cur] == -1) fail[cur] = gen(cur);
    cur = fail[cur];
    while (cur != 0) {
    ...
    if (adj[0][topar[node]] == -1) fail[node] = 0;
    else {
...
int main() {
    ...
    for (char c : tar) {
        ...
        node = adj[node][cur];
        if (wordidx[node] != -1) ans[wordidx[node]] = true;

        int trav = node;
        while (dict[trav] != -1) {
            trav = dict[trav];
            ans[wordidx[trav]] = true;
        }
    }

    for (int i = 0; i < k; i++) cout << (ans[i] or (seen[i] != -1 and ans[seen[i]]) ? "YES\n" : "NO\n");
