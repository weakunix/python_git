...
void setseg(int node, int l, int r, int a, int b, int label, int cnt) {
    prop(node, l, r, label != -2);
    if (l > b or r < a) return;
    if (l >= a and r <= b) {
        if (label != -2) seg[node].label = label;
...
int getseglabel(int node, int l, int r, int idx) {
    if (l > idx or r < idx) return -2;
    if (seg[node].label != -2 or l == r) return seg[node].label;
    int mid = l + (r - l) / 2;
...
void updpath(int a, int b, int label) {
    if (a == b) return;
    while (ht[tp[a]] >= ht[b]) {
        if (hv[a] != -1 and a != b) setseg(1, 0, n - 1, segidx[hv[a]], segidx[hv[a]], -2, 0);
...
int getpath(int a, int b) {
    ...
    while (ht[tp[a]] > ht[b]) {
        ...
        if (curlabel >= 0 and curlabel == getseglabel(1, 0, n - 1, segidx[par[tp[a]]])) res++;
        a = par[tp[a]];
    }
...
void subcase() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        if (type == 1) {
            updpath(a, l, i);
            updpath(b, l, i);
            setseg(1, 0, n - 1, segidx[l], segidx[l], -2, 0);
            if (hv[l] and (getlca(a, hv[l]) != hv[l] and getlca(b, hv[l]) != hv[l])) setseg(1, 0, n - 1, segidx[hv[l]], segidx[hv[l]], -2, 0);
        }
        else {
