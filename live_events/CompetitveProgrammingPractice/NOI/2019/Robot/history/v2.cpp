...
void lagrange(int l, int r, int cnt, int x) {
    if (not vis[rid[l][r]]) return;
    vis[rid[l][r]] = false;

    if (l > r) return;

    for (int i = max(l, (l + r) / 2 - 1 + (l + r) % 2); i <= min(r, (l + r) / 2 + 1); i++) {
        lagrange(l, i - 1, cnt, x);
        lagrange(i + 1, r, cnt, x);
    }
    ...
    for (int i = 1; i <= cnt; i++) {
        ...
        dp[rid[l][r]][0] = (dp[rid[l][r]][0] + dp[rid[l][r]][i] * cur % modval) % modval;
    }
