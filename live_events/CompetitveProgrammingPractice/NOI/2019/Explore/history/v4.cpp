...
void bsfind4(vector<int> range, vector<int> belong, bool isodd) {
    int len = range.size(), mid = len / 2;
    if (len == 1) {
        if (isodd) modify(range[0]);
        for (int i : belong) {
    ...
    bsfind4(lrange, lbelong, not isodd);
    bsfind4(rrange, rbelong, isodd);
    return;
}
...
void solve4() {
    ...
    int C = (n % 10 == 4 ? 100 : 1500);
    while (rem.size() > C) {
        int sz = max((int) rem.size() / 5, 1);
        sz = (rng() % sz + sz) % sz + 1;
        shuffle(all(rem), rng);

        vector<int> range, belong;
        for (int i = sz; i < rem.size(); i++) w[rem[i]] = query(rem[i]);
        for (int i = 0; i < sz; i++) {
            range.pb(rem[i]);
            modify(rem[i]);
        }
        ...
        vector<int> nwrem;
        for (int i : rem) if (not done[i]) nwrem.pb(i);
        swap(rem, nwrem);
    }
