        ...
        if (parent == -1) {
            if (m == n - 1) {
                ...
                for (Tree* i = nxt; i->rt != rt; i = i->nxt) {
                    int cur = i->rt;
                    c1 += p * (s[cur] + l) * (deg[cur] - 2) / (deg[cur] - 1);
                    p = p / (deg[cur - 1]);
                    l += nxtw;
                }
                ...
                for (Tree* i = prv; i->rt != rt; i = i->prv) {
                    int cur = i->rt;
                    c2 += p * (s[cur] + l) * (deg[cur] - 2) / (deg[cur] - 1);
                    p = p / (deg[cur - 1]);
                    l += prvw;
                }

                ans += s[node] * (deg[node] - 2) / deg[node] + (c1 + c2 + 2 * p * (s[node] + l)) / deg[node];
                for (simps& i : adj[node]) {
                    if (i.first == parent or badadj(i.first)) continue;
                    ak[node][i.first] = sk[node][i.first] * (deg[node] - 3) / (deg[node] - 1) + (c1 + c2 + 2 * p * (sk[node][i.first] + l)) / (deg[node] - 1);
                }
            }
        }
        else {
