        ...
        for (int j = 0; j <= i; j++) {
            ...
            for (int k = 0; k < (1 << cur); k++) {
                for (int l = 0; l < 2; l++) {
                    ...
                    ll curans = 1;
                    vector<vector<bool>> use(m, vector<bool>(4));
                    for (int y = 0; y < cur; y++) {
                        for (int x = 0; x < m; x++) {
                            if (i + robots[x].size() > n) continue;
                            if (k & (1 << y)) {
                                if (y < robots[x].size()) use[x][robots[x][y]] = true;
                                else use[x][0] = true;
                            }
                            if (l) use[x][0] = true;
                            if (j < i) use[x][0] = true;
                    ...
                    for (int x = 0; x < m; x++) {
                        if ((use[x][0] and use[x][1]) or (use[x][2] and use[x][3])) continue;
                        else if ((use[x][0] or use[x][1]) and (use[x][2] or use[x][3])) curans = (curans * 2) % modval;
                        else if ((use[x][0] or use[x][1]) or (use[x][2] or use[x][3])) curans = (curans * 3) % modval;
                    }
                    ...
                    for (int x = 0; x < m; x++) {
                        ...
                        for (int y = 0; y < cur; y++) {
                            if (k & (1 << y)) {
                                if (y < robots[x].size()) use[robots[x][y]] = true;
                                else use[0] = true;
                            }
                        }
                        if (l) use[0] = true;
