...
ll l, r, n, k, arr[N], dp[1 << R][R], ans = 1e18;
    ...
    for (int i = 1; i < (1 << r); i++) {
        ...
        for (int j = 0; j < r; j++) {
            ...
            for (int o = 0; o < r; o++) {
                ...
                ll t = dp[i][j], mv = (a + l - b) % l, cur = min((2 * k - t % k - 1) % k, mv);
                t += cur;
                mv -= cur;
                if (mv) {
                    mv = max(mv - 2, (ll) 0);
                ...
                if (dp[i ^ 1 << o][o] == -1) dp[i ^ 1 << o][o] = t;
                else setmn(dp[i ^ 1 << o][o], t);

                t = dp[i][j];
                mv = (b + l - a) % l;
                cur = min((2 * k - t % k - 1) % k, mv);
                t += cur;
                mv -= cur;
                if (mv) {
                    t++;
