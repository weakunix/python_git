...
simps& getnxt(int jmp, int idx, int x) {
    if (arr[idx - 2] == arr[idx - 1]) return nxt[idx][0][min(jmp, (int) nxt[idx][0].size() - 1)];
    x += arr[idx - 2] - arr[idx - 1] - 1;
    return nxt[idx][x][min(jmp, (int) nxt[idx][x].size() - 1)];
}
...
int main() {
    ...
    for (int i = 3; i <= n; i++) {
        if (arr[i - 2] == arr[i - 1]) {
            nxt[i].resize(1);
            nxt[i][0].pb(getjump(i, 0));
        }
        else {
            for (int j = 0; j < L; j++) nxt[i].resize(2 * (arr[i - 2] - arr[i - 1]));
            for (int j = -(arr[i - 2] - arr[i - 1]) + 1; j <= arr[i - 2] - arr[i - 1]; j++) nxt[i][j + arr[i - 2] - arr[i - 1] - 1].pb(getjump(i, j));
        }
    }
    ...
    for (int i = 1; i < L; i++) {
        for (int j = 3; j <= n; j++) {
            if (arr[j - 2] == arr[j - 1]) {
                ...
                if (cur.first <= n) {
                    cur = getnxt(i - 1, cur.first, cur.second);
                    nxt[j][0].pb(cur);
                }
            }
            else {
                for (int k = -(arr[j - 2] - arr[j - 1]) + 1; k <= arr[j - 2] - arr[j - 1]; k++) {
                    ...
                    if (cur.first <= n) {
                        cur = getnxt(i - 1, cur.first, cur.second);
                        nxt[j][k + arr[j - 2] - arr[j - 1] - 1].pb(cur);
                    }
                }
            }
        }
