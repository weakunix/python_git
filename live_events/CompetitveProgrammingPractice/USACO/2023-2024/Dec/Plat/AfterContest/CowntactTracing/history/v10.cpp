...
int n, q, k, ht[N], rt, centpar[N], sz[N], dist[N], nxt[N], best[N], mxdep[N], ans[Q], whenpsbl[N];
...
void dfs4(int node, int parent, int depth, int mx) {
    if (depth > k) return;
    setbest(nxt[node], best[min(mx, k - depth)]);
    for (int i : adj[node]) if (not vis[i] and i != parent) dfs4(i, node, depth + 1, mx);
    return;
}

void dfs5(int node) {
    fill(best, best + mxdep[node] + 1, -1);
    vis[node] = true;

    if (psbl[node]) best[0] = node;
    for (int i : adj[node]) if (not vis[i]) dfs3(i, -1, 1);

    for (int i = 1; i <= min(mxdep[node], k); i++) setbest(best[i], best[i - 1]);

    setbest(nxt[node], best[min(mxdep[node], k)]);
    for (int i : adj[node]) if (not vis[i]) dfs4(i, -1, 1, mxdep[node]);
...
int main() {
    ...
    for (int i = q - 1; i >= 0; i--) { //big k to small k, small psbl to big psbl
        for (; k > queries[i].first; k--) for (int j : setpsbl[k]) psbl[j] = true;
        assert(k == queries[i].first);
