...
int main() {
    ...
    for (int i = 1; i <= n; i++) {
        ...
        for (int j = 0; j < 8; j++) {
            int idx = upper_bound(all(loc[j]), i) - loc[j].begin();
            if (not idx) continue;
            ord.push_back({loc[j][idx - 1], j});
        }
        sort(rall(ord));
        ...
        for (int j = 0; j < 8; j++) {
            int idx = upper_bound(all(loc[j]), i) - loc[j].begin();
            if (idx >= loc[j].size()) continue;
            ord.push_back({loc[j][idx], j});
        }
        sort(all(ord));
