...
void stdfs(simps st, int node) {
    ...
    for (int i = 0; i < 10; i++) { //cur digit
        ...
        for (int l : adj[node]) { //child
            int childseq = st.second + i * curpw, cnt = st.first + 1; //what the child sees, how many he cares about
            while (dp[l][cnt].count(childseq) == 0) {
                childseq /= 10;
                cnt--;
            }
            partialdp[i][st.first][st.second] = partialdp[i][st.first][st.second] * dp[l][cnt][childseq] % modval;
        }
